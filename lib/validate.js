...
var parameters = function(apicall, opts, files) {
  ...
  for(i in apicall.parameters) {
    ...
    if (param.type=="date" && !_.isUndefined(opts[i]) && dateRegex.exec(opts[i])) {
      ...
      opts[i] = moment().add(parts[1], parts[0]).format("YYYY-MM-DD HH:mm:ss");
      
    }
    
    // apply a default
    if(_.isUndefined(opts[i]) && !_.isUndefined(param.default)) {

      // for dates
      if(param.type=="date" && param.default=="now") {
        opts[i] = moment().format("YYYY-MM-DD HH:mm:ss");
      } 
    ...
    switch(param.type) {
      ...
      case "date":
        ...
        if (_.isUndefined(opts[i]) && param.required === false) {
          break;
        }

        // now
        if(param.type=="date" && opts[i] == "now") {
          opts[i] = moment().format("YYYY-MM-DD HH:mm:ss");
        }
        
        else {
          opts[i] = moment(opts[i],"YYYY-MM-DD HH:mm:ss").format("YYYY-MM-DD HH:mm:ss");
        }
